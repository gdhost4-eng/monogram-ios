...
private func monogramSettingsEntries() -> [MonogramSettingsEntry] {
    ...
    footer(.messages, "Удалённые сообщения остаются в чате с пометкой 🗑. История изменений открывается из меню сообщения. В защищённых чатах можно копировать текст и сохранять медиа, а пересылка отправляет копию сообщения.")
